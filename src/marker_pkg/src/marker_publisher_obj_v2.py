...
class MarkerPublisher(object):
    def __init__(self):
        ...
        self.IGTL_REF_W_M                 = rospy.get_param('/igtl_ref_w_m', 0.707)
        self.IGTL_REF_X_M                 = rospy.get_param('/igtl_ref_x_m', 0.0)
        self.IGTL_REF_Y_M                 = rospy.get_param('/igtl_ref_y_m', 0.707)
        self.IGTL_REF_Z_M                 = rospy.get_param('/igtl_ref_z_m', 0.0)
        ...
        self.prev_pos                    = rospy.get_param('~/prev_pos', 500)
    ...
    def run_callback(self,data):
        ...
        if ((prev_pos+10)>len(self.path.poses)-1): pos_fin=len(self.path.poses)-1
        else: pos_fin=prev_pos+10
        ...
        if(self.prev_pos==len(self.path.poses)-1): #CONTROLLO PER LA PARTE FINALE IN UN PERCORSO
            ...
            #    theta=0.0
            #elif (self.euler_bottom[2]>math.radians(-89.0) and self.euler_bottom[2]<math.radians(-91.0)) :
            #    #cnt=cnt+1;
            #    theta=-90-math.degrees(b)
            #    #theta=math.degree(self.final_goal)/(2*cnt)

            if(self.euler_bottom[2]<-math.pi/2):
                theta=180.0+math.degrees(b)+math.degrees(self.euler_bottom[2])
            elif(self.euler_bottom[2]==-math.pi/2 and self.euler_bottom[2]!=-math.pi/2):
                print "________________________EL    IF______________________________"
                theta=math.degrees(b)+math.degrees(self.euler_bottom[2])
            elif(self.euler_bottom[2]<-math.pi/2+math.radians(1)):
                print "________________________EL    IF______________________________"
                theta=90.0+math.degrees(b)+math.degrees(self.euler_bottom[2])
            else:
            ...
            print "-_-_-_-_-_-_-_-_-_-------     "+ str(theta)
            print "EULER BOTTOM ---------- "+str(math.degrees(self.euler_bottom[2]))

        else:
            for a in numpy.arange(prev_a-1,prev_a+1,0.01):  #size marker?? ## CICLO PER L'ANGOLO
            #for a in numpy.arange(-55,55,0.1):  #size marker?? ## CICLO PER L'ANGOLO
                #print prev_a , prev_pos
                #print a

                for p in range (prev_pos-2, pos_fin+1):    ## MODIFICARE INSERENDO IL NUMERO MASSIMO DI SEQUENZE
	...
	beta = (beta + math.pi) % (2 * math.pi) - math.pi
        ...
        #elif(beta>math.pi and beta<2*math.pi): beta=-2*math.pi+beta

        self.error_pub.publish(math.degrees(beta))
        #self.error_pub.publish(A)
    ...
    def transform_callback(self,data):
        ...
        if self.bool_first:
            ...
            quat_init=numpy.quaternion(self.IGTL_REF_W_M,self.IGTL_REF_X_M,self.IGTL_REF_Y_M,self.IGTL_REF_Z_M).normalized()
