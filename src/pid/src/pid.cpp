...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  //MI STAMPA SEMPRE 0 e NON I VALORI NEL file .yaml
  node.getParam ("/Kp", Kp_);
  node.getParam ("/Ki", Ki_);
  node.getParam ("/Kd", Kd_);

  node.getParam ("/upper_limit", upper_limit_);
  node.getParam ("/lower_limit", lower_limit_);

  node.getParam ("/windup_limit", windup_limit_);
  node.getParam ("/cutoff_frequency", cutoff_frequency_);

  node.getParam("topic_from_controller", topic_from_controller_);
  node.getParam("topic_from_controller_side",topic_from_controller_side_);
  node.getParam("topic_from_plant", topic_from_plant_);
  node.getParam("setpoint_topic", setpoint_topic_);
  node.getParam("pid_enable_topic", pid_enable_topic_);
  node.getParam("max_loop_frequency", max_loop_frequency_);
  node.getParam("min_loop_frequency", min_loop_frequency_);
  node.getParam("pid_debug_topic", pid_debug_pub_name_);

  // Two parameters to allow for error calculation with discontinous value
  node.getParam("angle_error", angle_error_);
  node.getParam("angle_wrap", angle_wrap_);




/*
  node_priv.param<double>("Kp", Kp_, 0.02);
  node_priv.param<double>("Ki", Ki_, 0.04);
  node_priv.param<double>("Kd", Kd_, 0.0);
  node_priv.param<double>("upper_limit", upper_limit_, 5); //Corrisponde a 40 °
  node_priv.param<double>("lower_limit", lower_limit_, -5);
  node_priv.param<double>("windup_limit", windup_limit_, 5);
...
*/
  ...
  side_pub_ = node.advertise<std_msgs::String>(topic_from_controller_side_, 1);
  ...
  while( ros::ok() && !ros::topic::waitForMessage<std_msgs::Float64>(setpoint_topic_, ros::Duration(10.)))
     ROS_INFO("BBBBBBBBBBBBBBBBBBBBBBBBBBBBB");
  ...
  while( ros::ok() && !ros::topic::waitForMessage<std_msgs::Float64>(topic_from_plant_, ros::Duration(10.)))
     ROS_INFO("aaaaaaaaaaaaaaaaaaaaaaaaaa");
...
void PidObject::setpointCallback(const std_msgs::Float64& setpoint_msg)
{
  ...
  side_msg_.data=side_;
  side_pub_.publish(side_msg_);

  //setpoint_=abs(setpoint_); //per il controllore --> side_msg determina il regolatore da attivare
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    //p_theta=min(np.roots([467.5,-133.5, 10-setpoint_])) ;
    //v_theta=p_theta*5;
    double p_theta=0;  //Devo dichiarli qui?
    double v_theta=0;
    bool error_bool=false;
    
    if(setpoint_==0) p_theta=0;
      //p_theta=-0.000057689*(setpoint_)*(setpoint_)+0.0086*(setpoint_)+0.1908;
      //p_theta=-0.000057689*(setpoint_)*(setpoint_)+0.0086*abs(setpoint_)+0.1908; //PER 2 LATI
    if (setpoint_ < 0) {  
      p_theta = -9.277659174670159e-11 * pow(setpoint_, 6) 
                 + 2.490351106359760e-08 * abs(pow(setpoint_, 5)) 
                 - 2.621782720318396e-06 * pow(setpoint_, 4) 
                 + 1.382696731540563e-04 * abs(pow(setpoint_, 3)) 
                 - 0.003912423073244 * pow(setpoint_, 2) 
                 + 0.062468405150407 * abs(setpoint_) 
                 + 0.003724871350122;/*
	p_theta = -5.093287442619867e-11 * pow(setpoint_, 6) 
                 + 1.556773442246854e-08 * abs(pow(setpoint_, 5)) 
                 - 1.867826173426262e-06 * pow(setpoint_, 4) 
                 + 1.123902674784596e-04 * abs(pow(setpoint_, 3)) 
                 - 0.003605316845947 * pow(setpoint_, 2) 
                 + 0.062319853480703 * abs(setpoint_) 
                 + 0.005657364191576;*/

    }
      else if (setpoint_ > 0) {
      p_theta = -6.182828728433996e-11 * (setpoint_)*(setpoint_)*(setpoint_)*(setpoint_)*(setpoint_)*(setpoint_)+ 1.760100704070713e-08 * abs((setpoint_)*(setpoint_)*(setpoint_)*(setpoint_)*(setpoint_)) - 1.948712480267485e-06 * (setpoint_)*(setpoint_)*(setpoint_)*(setpoint_) + 1.066331280353382e-04 * abs((setpoint_)*(setpoint_)*(setpoint_)) - 0.003060165907763 * (setpoint_)*(setpoint_) + 0.047412133923232 * abs((setpoint_)) + 0.012411980088755;/*
	p_theta = 1.104215358833055e-12 * pow(setpoint_, 6) 
                 + 2.039812885139858e-10 * abs(pow(setpoint_, 5)) 
                 - 1.368138472131778e-07 * pow(setpoint_, 4) 
                 + 1.993565296879140e-05 * abs(pow(setpoint_, 3)) 
                 - 0.001273087890311 * pow(setpoint_, 2) 
                 + 0.039789712015967 * abs(setpoint_) 
                 - 0.004299510321841;*/
    }
    std::cout << "setpoint"<< setpoint_  << std::endl;
    std::cout << "p_theta: "<< p_theta << std::endl;

    
    v_theta=p_theta*5;
    //std::cout << "V_THETA: "<<v_theta << std::endl;
    //std::cout << "SET_POINT: "<<setpoint_ << std::endl;
    //std::cout << "ERROR: "<<error_.at(0) << std::endl;
/*
    while (error_.at(0)>-0.5 && error_.at(0)<0.5){

      control_effort_pub_.publish(control_msg_);
      std::cout << "CICLO WHILE: "<< control_msg_ << std::endl;

    }
*/
    if (error_.at(0)>-0.4 && error_.at(0)<0.4){
        error_bool=true;
    }
    else
      error_bool=false;
...
/*  //NOTCH FILTER CON LIBRERIA ESTERNA (NON FUNZIONA)
    double fsfilt=1000.0;
    double gb=1.0;
    double Q=100.0;
    double fstep=10.0;
    short fmin=50.0;
    ...
    br_iir_init(fsfilt, gb, Q, fstep,  fmin);
    br_iir_setup(H,ind);
    y_notch=br_iir_filter(control_effort_, H);
*/

/*
    //NOTCH FILTER (Funziona)

    std::vector<double> b_ (3,0), a_ (2,0), control_effort_in_ (3,0), notch_control_(3,0);
    ...
    double BW= 0.03;
    double R=1-3*BW;
    //double K=(1-2*R*cos(2*PI*f0/fs)+R*R)/(2-2*cos(2*PI*f0/fs));
    R=0.8;
    double K=1;
    ...
    b_.at(0)=K;
    b_.at(1)=-2*K*cos(2*PI*f0/fs);
    b_.at(2)=K;

    a_.at(0)=2*R*cos(2*PI*f0/fs);
    a_.at(1)=-R*R;

    control_effort_in_.at(2)=control_effort_in_.at(1);
    control_effort_in_.at(1)=control_effort_in_.at(0);
    control_effort_in_.at(0)=control_effort_;

    notch_control_.at(2)=notch_control_.at(1);
    notch_control_.at(1)=notch_control_.at(0);
    notch_control_.at(0)=a_.at(0)*notch_control_.at(1)+a_.at(1)*notch_control_.at(2)+
                            b_.at(0)*control_effort_in_.at(0)+b_.at(1)*control_effort_in_.at(1)+b_.at(2)*control_effort_in_.at(2);


    control_effort_=notch_control_.at(0);
...
*/
    ...
    if (pid_enabled_)
    {
      //control_msg_.data = error_.at(0)*Kp_;
      if(error_bool==true) {
        std::cout << "Control_msg_: "<<control_msg_ << std::endl;
        control_effort_pub_.publish(control_msg_);
        std::cout << "Control_msg_: " << std::endl;}
      else{

        if(setpoint_==0){ control_effort_=0;}

        else if (setpoint_>0){
          //control_msg_.data = control_effort_; // senza FEED FORWARD
          control_msg_.data = control_effort_+ v_theta; // CON FEED FORWARD
          //std::cout << "Control_msg_data: "<<control_msg_.data << std::endl;
          //control_msg_.data = v_theta; // SOLO FEED FORWARD er stond 0.85*v_theta


        }
        else if (setpoint_<0){
          control_msg_.data = control_effort_- v_theta; // CON FEED FORWARD stond -0.85 * v_theta
          //control_msg_.data = control_effort_;
	  //control_msg_.data = v_theta; // SOLO FEED FORWARD stond -0.83 * v_theta
