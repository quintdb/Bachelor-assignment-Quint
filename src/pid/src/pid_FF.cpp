...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<double>("upper_limit", upper_limit_, 2.2); //Corrisponde s 40 °
  node_priv.param<double>("lower_limit", lower_limit_, -2.2);
  node_priv.param<double>("windup_limit", windup_limit_, 2.2);
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    p_theta=min(np.roots([467.5,-133.5, 10-theta])) ;
    v_theta=p_theta*5;



    /*while (error_.at(0)>-0.2 && error_.at(0)<0.2){
    ...
    if (pid_enabled_)
    {
      control_msg_.data = control_effort_+v_theta;
