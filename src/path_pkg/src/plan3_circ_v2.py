...
def main():
    ...
    resolution = rospy.get_param('~resolution', 0.0015)
    ...
    for t in numpy.arange(0.0 , 2*math.pi+1,resolution):
        ...
        if(t>2*math.pi):
            #t=t-2*math.pi
            x=math.cos(-3*math.pi/2)+0.5+t-2*math.pi
            y=old_y
        ...
        #if(t>2*math.pi):
        #    yaw=math.pi/2-math.radians(0.1)
